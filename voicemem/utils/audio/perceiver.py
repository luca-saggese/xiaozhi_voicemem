...
class AudioPerceiver:
    ...
    def _env_detector(self):
        with self._lock:
            if "env_detector" not in self._cache:
                from voicemem.utils.audio.environment.environment_detector_ast import (
                    ASTEnvironmentDetector,
                )
    ...
    def _clap_env_detector(self):
        with self._lock:
            if "clap_env_detector" not in self._cache:
                from voicemem.utils.audio.environment.environment_detector_clap import (
                    CLAPEnvironmentDetector,
                )
    ...
    def _emotion_detector(self):
        with self._lock:
            if "emotion_detector" not in self._cache:
                ...
                from voicemem.utils.audio.emotion.paper_emotion_detector import (
    ...
    def preprocess(
        ...
        if person_id is None and speaker and speaker != "Speaker 0":
            try:
                from voicemem.utils.audio.voiceprint.speaker_identity import (
                    parse_self_identification,
                )
                self_name = parse_self_identification(text)
                if self_name:
                    registry = self._registry()
